...
public class AreaUtils {
    ...
    static {
        ...
        try {
            Files.readAllLines(Paths.get(ResourceUtils.getResource("area.csv").toURI()))
                    .forEach(line -> {
                        ...
                        all.add(items);
                        Area area = new Area(Integer.valueOf(items[0]), items[1], Integer.valueOf(items[2]),
                                null, new ArrayList<>());
                        // 添加到 areas 中
    ...
    public static String format(Integer id, String separator) {
        ...
        for (int i = 0; i < AreaTypeEnum.values().length; i++) { // 避免死循环
            ...
            area = area.getParent();
            if (area == null ||
                    ObjectUtils.equals(area.getId(), Area.ID_GLOBAL) ||
    ...
    public static <T> List<T> getByType(AreaTypeEnum type, Function<Area, T> func) {
        return areas.values().stream().filter(area -> type.getCode() == area.getType()).map(func).filter(Objects::nonNull).collect(Collectors.toList());
    }
