...
public class JacksonUtils {
	...
	public static String toStringWithView(Object value, Class<Views.Public> serializationView) throws JsonProcessingException {
					...
					if (node.has(token)) {
						...
						if (task.isLast()) {
							...
							for (Map.Entry<String, String> replacement : task.getVariables().entrySet()) {
								name = name.replace("$" + replacement.getKey(), StringUtils.defaultString(replacement.getValue(), StringPool.EMPTY));
							}
