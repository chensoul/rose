...
public class RetOps<T> {
	 ...
	 * @throws Ex 断言失败时抛出
	 */
	public <Ex extends Exception> RetOps<T> assertSuccess(Function<? super RestResponse<T>, ? extends Ex> func) throws Ex {
	 ...
	 * @throws Ex 断言失败时抛出
	 */
	public <Ex extends Exception> RetOps<T> assertHasData(Function<? super RestResponse<T>, ? extends Ex> func) throws Ex {
	...
	public <U> RetOps<U> map(Function<? super T, ? extends U> mapper) {
		RestResponse<U> result = RestResponse.build(mapper.apply(original.getData()), original.getCode(), original.getMessage());
		return of(result);
	}
	...
	public void acceptIf(Consumer<? super T> consumer, int... codes) {
		acceptIf(o -> Arrays.stream(codes).filter(c -> original.getCode() == c).findFirst().isPresent(), consumer);
	}
}
