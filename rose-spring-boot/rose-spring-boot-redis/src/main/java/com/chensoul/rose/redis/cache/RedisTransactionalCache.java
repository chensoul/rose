...
public abstract class RedisTransactionalCache<K extends Serializable, V extends Serializable>
    ...
    static final byte[] BINARY_NULL_VALUE =
            org.springframework.data.redis.serializer.RedisSerializer.java().serialize(NullValue.INSTANCE);
    ...
    private final org.springframework.data.redis.serializer.RedisSerializer<String> keySerializer =
            StringRedisSerializer.UTF_8;

    private final RedisSerializer<K, V> valueSerializer;

    public RedisTransactionalCache(
            String cacheName,
    ...
    @Override
    public <R> R getAndPutInTransaction(
            K key,
            Supplier<R> dbCall,
        ...
        return TransactionalCache.super.getAndPutInTransaction(
                key, dbCall, cacheValueToResult, dbValueToCacheValue, cacheNullValue);
    }
    ...
    protected RedisConnection getConnection(byte[] rawKey) {
        ...
        Jedis jedis = new Jedis(String.valueOf(
                (((JedisClusterConnection) connection).getNativeConnection().getConnectionFromSlot(slotNum))));
    ...
    protected void executeScript(
        ...
        try {
            ...
            log.debug(
                    "Loading LUA with expected SHA [{}], connection [{}]",
                    new String(scriptSha),
