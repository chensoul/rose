...
public abstract class VersionedRedisCache<K extends VersionedCacheKey, V extends Serializable & HasVersion>
    ...
    static final byte[] SET_VERSIONED_VALUE_SHA =
            StringRedisSerializer.UTF_8.serialize("0453cb1814135b706b4198b09a09f43c9f67bbfe");

    private static final int VERSION_SIZE = 8;

    private static final int VALUE_END_OFFSET = -1;

    public VersionedRedisCache(
            String cacheName,
    ...
    private void doPut(byte[] rawKey, V value, Long version, Expiration expiration, RedisConnection connection) {
        ...
        byte[] rawExpiration =
                StringRedisSerializer.UTF_8.serialize(String.valueOf(expiration.getExpirationTimeInSeconds()));
        executeScript(
                connection,
                SET_VERSIONED_VALUE_SHA,
