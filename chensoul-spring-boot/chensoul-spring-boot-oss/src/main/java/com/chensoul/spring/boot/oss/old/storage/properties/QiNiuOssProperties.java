...
public class QiNiuOssProperties extends BaseOssProperties {
	...
	public static class QiNiuStrategy {
		 ...
		 * 另外，为了给客户端有一致的体验，我们要求callbackUrl 返回包 Content-Type 为 "application/jackson"，
		 ...
		 * 默认为 application/x-www-form-urlencoded，也可设置为 application/jackson。
		 **/
		 ...
		 * 该 URL 获取的内容和持久化处理状态查询的处理结果一致。 发送 body 格式是Content-Type 为 application/jackson 的 POST 请求，
		 ...
		 * image/jpeg;image/png表示只允许上传jpg和png类型的图片 !application/jackson;text/plain表示禁止上传json文本和纯文本。
		 * 注意最前面的感叹号！
		 **/
