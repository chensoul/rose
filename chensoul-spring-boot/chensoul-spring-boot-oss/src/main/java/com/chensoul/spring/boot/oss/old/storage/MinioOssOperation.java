...
import com.chensoul.spring.boot.oss.old.storage.domain.StorageResponse;
...
public class MinioOssOperation implements OssOperation {
	...
	private List<StorageItem> getStorageItems(Iterable<Result<Item>> iterable) {
		return StreamSupport.stream(iterable.spliterator(), true).map(itemResult -> {
			try {
				...
				throw new StorageException(BaseOssProperties.StorageType.MINIO, "Error while parsing list of objects",
					e);
			}
	...
	public List<StorageItem> list() {
		Iterable<Result<Item>> iterable = minioClient.listObjects(
			ListObjectsArgs.builder().bucket(properties.getBucket()).region(properties.getRegion()).build());
	...
	public StorageResponse upload(StorageRequest request) {
		try {
			...
			final ObjectWriteResponse object = minioClient.putObject(PutObjectArgs.builder()
				.bucket(bucket)
				.object(fileName)
