...
import com.chensoul.ip.utils.IpRegionUtils;
...
public class SysLogUtils {
	...
	public SysLogInfo getSysLog(ProceedingJoinPoint joinPoint, SysLog sysLog) {
		...
		if (sysLog.request()) {
			...
			if (requestOptional.isPresent()) {
				...
				sysLogInfo.setRemoteIp(WebUtils.getRemoteIp(request));
				sysLogInfo.setRemoteLocation(IpRegionUtils.getArea(sysLogInfo.getRemoteIp()).getName());
	...
	private boolean isFilterObject(Object o) {
		if (Objects.isNull(o) || o.getClass().isAnnotationPresent(SysLogIgnore.class)
